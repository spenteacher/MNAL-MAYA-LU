...
\subsection{Ejemplo de Uso en el Código}

El método clave que usa la descomposición LU está en \texttt{physics\_simple.py}:
...
def update(self):
    ...
    if self.use_lu:
        if not self.K_decomposed:
            self.solver.decompose(K)  # Solo primera vez
        ...
        x_new = self.solver.solve(K, f_x)  # Gauss cada vez
        y_new = self.solver.solve(K, f_y)
\end{verbatim}

La diferencia está en que el método LU descompone \(K\) una sola vez (línea 5) y reutiliza esa factorización (línea 8-9), mientras que Gauss resuelve desde cero cada frame.
...
Este trabajo ha demostrado de forma rigurosa que:

\begin{enumerate}
\item La descomposición LU es una herramienta fundamental para resolver eficientemente múltiples sistemas lineales con la misma matriz.

\item El análisis de complejidad muestra que para \(m\) sistemas con matriz \(A \in \mathbb{R}^{n \times n}\), el speedup teórico es:
\[
S = \frac{m}{1 + \frac{3m}{n}}
\]
que para \(m \gg 1\) se aproxima a \(n/3\).

\item La formulación de sistemas físicos como problemas de álgebra lineal permite aplicar estas técnicas sin recurrir a ecuaciones diferenciales.

\item Los resultados experimentales confirman las predicciones teóricas, mostrando speedups de 2-3x en sistemas típicos.
\end{enumerate}

\subsection{Aplicaciones}

Las técnicas desarrolladas son aplicables a diversos campos:

\begin{itemize}
\item \textbf{Análisis estructural}: Cálculo de deformaciones en vigas, puentes
\item \textbf{Circuitos eléctricos}: Análisis de redes con diferentes condiciones
\item \textbf{Gráficos computacionales}: Simulación de telas, cabello, objetos deformables
\item \textbf{Métodos iterativos}: Precondicionadores basados en LU incompleta
\item \textbf{Optimización}: Resolución de sistemas de ecuaciones normales
\end{itemize}
...
\begin{itemize}
\item El modelo asume pequeñas deformaciones (linealización)
\item No se consideran efectos dinámicos (inercia, oscilaciones)
\item La matriz \(K\) debe permanecer constante para reutilizar LU
\end{itemize}

\textbf{Extensiones posibles}:
\begin{enumerate}
\item \textbf{Matrices sparse}: Algoritmos especializados para estructuras ralas
\item \textbf{Factorización de Cholesky}: Para matrices simétricas definidas positivas, reducir coste a \(n^3/3\)
\item \textbf{Actualización de rango bajo}: Modificar \(L\) y \(U\) cuando \(A\) cambia ligeramente
\item \textbf{Paralelización}: Implementación en GPU para sistemas masivos
\item \textbf{Precondicionadores}: LU incompleta para métodos iterativos
\end{enumerate}

\subsection{Reflexión Final}

La descomposición LU ejemplifica cómo el análisis matemático riguroso conduce a algoritmos computacionales eficientes. La separación entre factorización (costosa) y resolución (económica) es un paradigma que aparece en muchos contextos: SVD, factorización QR, eigendescomposición, etc.

Este trabajo demuestra que conceptos avanzados de álgebra lineal numérica pueden implementarse y visualizarse usando únicamente matemáticas de segundo curso, sin recurrir a ecuaciones diferenciales, proporcionando una base sólida para estudios posteriores en análisis numérico y computación científica.
